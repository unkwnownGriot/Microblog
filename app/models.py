...
#Table d'association
# Garde en tete que sa c'est le module sqlalchemy et db la database
followers = sa.Table(
    'followers',
    db.metadata,
...
class User(db.Model,UserMixin):
    ...
    # getting users that you are following (left side) 
    following: so.WriteOnlyMapped['User'] = so.relationship(
        secondary=followers, primaryjoin=(followers.c.follower_id == id ),
        secondaryjoin=(followers.c.followed_id == id), back_populates='followers'
    )
    #getting the users who are following you (right side)
...
class Post(SearchableMixin,db.Model):
    ...
    lang_post : so.Mapped[Optional[str]] = so.mapped_column(sa.String(10)) 
